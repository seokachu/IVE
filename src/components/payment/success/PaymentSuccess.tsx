...
const PaymentSuccess = () => {
  ...
  useEffect(() => {
    if (!orderId || !paymentKey || !amount || !address || !session || !cartItems || payment || isDataSaved) return;
    ...
    const processPayment = async () => {
      try {
        ...
        //주문 상품 데이터
        const orderItemsData = cartListItemsToOrderItems(cartItems, checkoutItems, session.user.id, orderId);
        ...
        //결제한 장바구니 데이터 비우기
        const updatedCart = cartItems.filter((item) => !checkoutItems.includes(item.id));
