...
const BoardListItem = ({ item, keyword }: BoardListItemProps) => {
  ...
  return (
    <li onClick={onClickBoardDetail} className="cursor-pointer first:border-t lg:first:border-t-0 lg:h-[50px] border-b">
      {/* Desktop */}
      <div className="hidden lg:flex text-center py-3 hover:bg-gray-50 items-center">
        ...
        <div className="w-[40%] text-left flex gap-1">
          <p className="text-left max-w-[80%] truncate">{highlightKeyword(item.title, keyword || "")}</p>
          <p className="text-blue-500" aria-label={`댓글 ${item.board_comments[0]?.count || 0}개`}>
            &#91;{item.board_comments[0]?.count || 0}&#93;
          </p>
        </div>
        <h3 className="w-[15%] text-left pl-3 flex items-center gap-1">
          {item.user.avatar_url && (
            <UserAvatar size="xs" userId={item.user_id} userName={item.user.name} avatarUrl={item.user.avatar_url} />
          )}
          <span>{item.user.name}</span>
        </h3>
        <time className="w-[15%] text-gray-500">{formatDate(item.created_at, "dash")}</time>
        <p className="w-[10%] text-gray-500">{item.views || 0}</p>
        <p className="w-[10%] text-gray-500">{item.board_likes[0]?.count || 0}</p>
      </div>
      {/* Mobile */}
      <div className="lg:hidden text-center py-3 px-5 hover:bg-gray-50">
        <div className="flex items-center justify-between gap-5">
          <div className="flex flex-col gap-2 flex-1 min-w-0">
            <p className="w-full truncate text-left">{highlightKeyword(item.title, keyword || "")}</p>
